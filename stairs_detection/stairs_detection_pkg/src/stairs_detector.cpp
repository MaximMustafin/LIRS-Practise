...
bool Stairs_Detector::getStairs(const cv::Mat& input_image, std::vector<cv::Point>& bounding_box) {
	...
	if ( input_image.channels() == 1) {
		...
		cv::cvtColor(src_gray, src_rgb, cv::COLOR_GRAY2RGB);
	} else {
 ...
 */
void Stairs_Detector::setParam(const Stairs_Detector_Params& param) {
	param_ = param;
...
void Stairs_Detector::drawLines(cv::Mat &image, const Lines &lines, const cv::Scalar &color) {
 ...
 */
void Stairs_Detector::drawLinesSlope(cv::Mat &image, const Lines &lines, const cv::Scalar &color) {
	for (int i = 0; i < lines.size(); i++) {
		putText(image, std::to_string(lines[i].t), lines[i].p1, cv::FONT_HERSHEY_SIMPLEX, 0.3, cv::Scalar(255, 255, 0));
	}
}
 ...
 * @param[in]  color  The color
 */
void Stairs_Detector::drawLinesRadius(cv::Mat &image, const Lines &lines, const cv::Scalar &color) {
	for (int i = 0; i < lines.size(); i++) {
		putText(image, std::to_string(lines[i].b), lines[i].p1, cv::FONT_HERSHEY_SIMPLEX, 0.3, cv::Scalar(255, 255, 0));
	}
}

void Stairs_Detector::drawBox(cv::Mat& image, const std::vector<cv::Point>& bounding_box) {
 ...
 */
void Stairs_Detector::visualizeHist(const std::vector<int> hist, int height, std::string window_name) {
 ...
 */
void Stairs_Detector::houghLine(const cv::Mat &edge_image, Lines &lines) {
	lines.clear();
...
void Stairs_Detector::filterLinesBySlopeThreshold(const Lines& input_lines, Lines& filtered_lines ) {
...
void Stairs_Detector::filterLinesBySlopeHist(const Lines& input_lines, Lines& filtered_lines ) {
	// calculate histogram
 ...
 */
bool Stairs_Detector::getBoundingBox(const cv::Mat input_rgb_image, const Lines &input_lines, std::vector<cv::Point>& bounding_box) {
	bounding_box.clear();
 ...
 */
bool Stairs_Detector::getNeighbourId(int row, int col, std::vector<cv::Vec2i> &ids, const cv::Mat& input_image) {
 ...
 */
bool Stairs_Detector::isLineClose(const Line &line1, const Line &line2) {
 ...
 */
bool Stairs_Detector::isLineParallel(const Line &line1, const Line &line2) {
 ...
 * @return     void
 */
void Stairs_Detector::getMergePointList(const Lines &input_lines,
 ...
 */
void Stairs_Detector::mergeLines(const cv::Mat& input_image, const Lines &input_lines, Lines &merged_lines) {
	std::vector<std::vector<cv::Point>> merge_points_list;
	...
	while (merge_points_list.size() > 0) {
		...
		for (int i = 0; i < merge_points_list.size(); i++) {
			...
			cv::fitLine(merge_points_list[i], merged_vline, cv::DIST_L2, 1.3, 0.01, 0.01);
...
void Stairs_Detector::ignoreInvalid(const cv::Mat& input_image, cv::Mat& filter_image) {
 ...
 */
void Stairs_Detector::cannyEdgeDetection(const cv::Mat& input_image, cv::Mat &edge) {
 ...
 * @param      edge  The edge
 */
void Stairs_Detector::sobelEdgeDetection(const cv::Mat& input_image, cv::Mat &edge) {
	cv::blur(input_image, edge, cv::Size(3, 3));
 ...
 * @param      edge  The edge
 */
void Stairs_Detector::laplacianEdgeDetection(const cv::Mat& input_image, cv::Mat &edge) {
 ...
 */
void Stairs_Detector::fillByNearestNeighbour(const cv::Mat& input_image, cv::Mat& filled_image) {
 ...
 */
bool Stairs_Detector::isInbound(int x, int y, const cv::Mat& image) {
	if (x < 0) {
		return false;
	}
	if (y < 0) {
